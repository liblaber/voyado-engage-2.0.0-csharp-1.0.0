...
{
    internal TransactionsService(HttpClient httpClient)
        : base(httpClient) { }
    ...
    /// </summary>
    public async Task<object> ImportTransactionsImportReceiptsAsync(
        Receipt input,
    ...
    )
    {
        ...
        var request = new RequestBuilder(HttpMethod.Post, "api/v2/receipts")
    ...
    /// </summary>
    /// <param name="input">The JSON object which contains the receipts/transactions</param>
    public async Task<object> ImportTransactionsImportAsync(
        ImportTransactionsObject input,
        CancellationToken cancellationToken = default
    )
    {
        ArgumentNullException.ThrowIfNull(input, nameof(input));

        var request = new RequestBuilder(HttpMethod.Post, "api/v2/transactions")
            .SetContentAsJson(input, _jsonSerializerOptions)
            .Build();
